...
import config
import custom_tools
import mitres_make_t5_template as make_t5_template
...
exp_folder = f"../mitrest_{make_t5_template.filter_limit}_runs/{config.params['TEMPLATE']}/{date_time}"
Path(exp_folder).mkdir(parents=True, exist_ok=True)

import wandb

wandb.init(project=f"mitrest-res-size-{make_t5_template.filter_limit}", entity="manoharupv")
